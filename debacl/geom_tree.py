...
class GeomTree(object):
	...
	def plot(self, form, width='uniform', sort=True, gap=0.05, 
		color_nodes=None):
		"""
...
def geomTree(X, k, gamma, n_grid=None, approx=True, verbose=None):
	"""
	...
	verbose: int, optional
		Number of iterations between printing updates to the screen. Default is
		None, which prints no updates.
	...
	neighbors, k_radius = utl.knnGraph(X, k, approx=approx)
	fhat = utl.knnDensity(k_radius, n, p, k)
	bg_sets, levels = utl.constructDensityGrid(fhat, mode='mass', n_grid=n_grid)
	T = constructTree(neighbors, levels, bg_sets, mode='density', verbose=verbose)		
	T.prune(method='size-merge', gamma=gamma)
		
	return T


def constructTree(neighbors, levels, bg_sets, mode='general', verbose=None):
	"""
